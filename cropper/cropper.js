...
(function (factory) {
  ...
  function Cropper(element, options) {
    ...
    this.load(this.options.imageUrl);
  }
  ...
  prototype.load = function (url) {
    ...
    if (options.checkImageOrigin && isCrossOriginURL(url)) {
      ...
      if (!$this.prop('crossOrigin')) { // Only when there was not a "crossOrigin" property
        bustCacheUrl = url; // Bust cache (#148)
      }
    }
  ...
  $.extend(prototype, {
    ...
    initContainer: function () {
      ...
      $cropper.css((this.container = {
        width: max($container.width(), num(options.minContainerWidth) || 10),
        height: max($container.height(), num(options.minContainerHeight) || 10)
      }));
    ...
    renderImage: function () {
      ...
      this.$clone.css(getTransforms(image));
    },
  ...
  prototype.preview = function () {
    ...
    this.$viewBox.find('img').css(getTransforms(image));
    ...
    this.$preview.each(function () {
      ...
      $this.width(newWidth).height(newHeight).find('img').css(getTransforms(image));
    });
  };
  ...
  $.extend(prototype, {
    ...
    dragstart: function (event) {
      ...
      dragType = dragType || $(e.target).data('drag') || 'move';
  ...
  $.extend(prototype, {
    ...
    moveCanvas: function (data) {
      var canvas = this.canvas;

      if (this.built && !this.disabled && data) {
        if (isNumber(data.top)) canvas.top = data.top;
        if (isNumber(data.left)) canvas.left = data.left;
        this.renderCanvas(true);
      }
    },
    ...
    resizeCanvas: function (data) {
      ...
      if (this.built && !this.disabled && data) {
        if (isNumber(data.top)) canvas.top = data.top;
        if (isNumber(data.left)) canvas.left = data.left;
        if (isNumber(data.width)) canvas.width = data.width;
        if (isNumber(data.height)) canvas.height = data.height;
        this.renderCanvas(true);
      }
    },
    ...
    zoom: function (delta) {
      ...
      if (delta && this.built && !this.disabled && this.options.zoomable) {
        ...
        if (delta > 0) {
          ...
          if (imageData.width / cropBox.width > 2) {
    ...
    getData: function () {
      ...
      data.scaleX = image.scaleX;
      data.scaleY = image.scaleY;
      data.ratio = ratio;
      return data;
    },
    ...
    setData: function (data) {
      ...
      if (this.built && !this.disabled && $.isPlainObject(data)) {
        ...
        if (isNumber(data.scaleX) && isNumber(data.scaleY)) {
  ...
  })('<0 6="5-container"><0 6="5-canvas"><0 6="5-2-9" 3-2="move"></0></0><0 6="5-crop-9"><1 6="5-view-9"></1><1 6="5-8 8-h"></1><1 6="5-8 8-v"></1><1 6="5-face" 3-2="all"></1></0></0>', 'div,span,drag,data,point,cropper,class,line,dashed,box');
